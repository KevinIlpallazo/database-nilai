...
<h1 style="text-align:center">Nilai Siswa</h1><hr/>
  ...
  $data1 = mysqli_query($conn,"select * from biodata");

    while ($r = mysqli_fetch_array($data) AND $d = mysqli_fetch_array($data1)) {
        $NNama = $d['nikmat_nama'];
        $NNis = $d['nikmat_nis'];
        $NRombel = $d['nikmat_rombel'];
        $NProd = $r['nilai_prod'];
        $NBi = $r['nilai_bindo'];
        $NSb = $r['nilai_sbud'];
        $NMtk = $r['nilai_mtk'];
...
?>
  ...
  ?>
    ...
    <h1 style="text-align:center">Kelompok </h1>
<ul class="list-group">
